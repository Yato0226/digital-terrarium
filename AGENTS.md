...
- Reproduction: pawns carry `sex` (M/F). `Mate` (cost 10) succeeds only on the same tile with an opposite-sex active pawn whose relationship to the suitor is ≥ 25 (`MATE_RELATIONSHIP`); a starving female can't conceive and the colony caps at `MAX_PAWNS` (10). On a 50% roll the female gains `pregnant_ticks` (6); `tick_environment` counts pregnancy down and calls `_give_birth` (newborn: weaker HP/energy, `child_ticks` 5 before it can mate, auto name/job/sex from the state pools). Children can't court or be courted; a birth that comes due while the colony is at cap re-delays the pregnancy rather than losing it.
