...
- Actions are exact strings `"Chop" | "Rest" | "Scout" | "Attack" | "Forage" | "Build" | "Share" | "Move"`. `target` is only valid for Attack or Share (same/adjacent tile); `direction` (N/S/E/W) only for Move; engine rejects missing/self/down/unknown/far targets.
...
- **Adding a stat/resource touches all of:** `state.make_pawn` + `state._migrate_pawn` (old saves auto-migrate), `state.DEFAULT_BIOME` for biome fields, `prompts.build_prompt`, `engine` resolution, `core.post_to_discord`, and optionally `bot.god_edit`. Adding an **action** touches `schema.ACTIONS` (Literal), `engine.ACTION_COSTS`, and `bot.order`'s whitelist in lockstep.

## State & persistence

- `world_state` in `state.py`: `{tick, history, biome, graveyard, grid, pawns}`. Pawn shape is nested (`vitals.hp`, `inventory.{wood,food,stone,fiber}`, `gear.{main_hand,body}`, `counters.*`, `title`, `job`, `pos`, `mental_break`, `skills.*`, `relationships.*`, `status`), not flat. `biome` holds shared environment state (`season`, `weather`, `day`, `campfire`, `shelter`, `wood_stock`, `food_stock`); `grid` holds the 5×5 tile map. Environmental death (Blizzard freeze or `starving_ticks > 5`) removes a pawn into `graveyard` snapshots (`{id, name, title, cause, died_tick, born_tick, epitaph}`) — combat 0 HP stays recoverable `incapacitated`.
...
- `core.tick_lock` serializes ticks and state mutation. **Any Discord command that mutates `world_state` must wrap its body in `async with core.tick_lock:`** (see `bot.py`).
- `pause_event` gates the scheduler; single `tick_loop` task is created in `bot.on_ready` with a reconnect guard — don't add a second one.
