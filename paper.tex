...
As the colony's history accumulates, it becomes a culture: a \texttt{traditions} block in \texttt{world\_state} tallies colony-wide counters --- predators slain, trees felled, and rations shared --- that survive individual pawn deaths. On each season change the engine's \texttt{\_evaluate\_tradition} checks the thresholds (10, 100, 20, in priority order) and, once one is crossed, permanently stamps a \textbf{Tradition Tag}: \emph{Hunters of the North} (double combat XP on hunting kills, and cold-weather penalties reduced by 1), \emph{Children of the Forest} ($+1$ wood yield from every \texttt{Chop}, and shelter damage from fires and mental breaks halved --- the only ways shelter degrades), or \emph{Kindred of the Hearth} (social \texttt{Interact} grants $+8$ morale instead of $+5$). Tags are sticky once earned, appear in the prompt and the Discord embed, and are inspected via the \texttt{!tradition} god command, which also shows the live counters en route to the next tag.

Culture also has its rituals. On the first tick of a new Winter or Summer, if the colony larder exceeds 15 \texttt{food\_stock}, the engine's \texttt{\_seasonal\_feast} spends 5 of it on a \textbf{Solstice Feast}: every active pawn gains $+15$ morale and a \texttt{Festive} moodlet ($+5$ for 15 ticks). And when a pawn dies, \texttt{\_kill} marks the graveyard entry as \emph{beloved} whenever survivors held it in high average regard ($\geq 25$ relationships); thereafter any pawn on the Camp or Ruins tile can \texttt{Interact} with a ``bury'' / ``mourn'' / ``eulogize'' flavor to hold a \textbf{funerary rite} that halves the remaining \texttt{Grief} moodlet duration ($\lfloor \texttt{ticks\_left}/2 \rfloor$) for everyone gathered there --- a communal counterweight to the automatic $+10$ grief of loss.
...
Prosperity, however, draws wolves of another kind. In Autumn, once the colony holds $\geq 30$ combined food+wood across its stores and rucksacks, the engine's \texttt{tick\_environment} spawns a \textbf{Scavenger Raid}: 1--2 hostile \texttt{scavenger\_N} entities appear at random edge tiles and march greedily toward the Camp (\texttt{\_step\_raiders}). A standing palisade slows their approach --- each raider carries a \texttt{slowed} counter set to the palisade level between moves. On the Camp tile they must first run the colony's defenses: every tamed Wolf/Bear at the camp intercepts, biting the intruder for its \texttt{bite\_damage}, and the highest-combat pawn on the Camp tile counter-strikes ($4 + \texttt{combat}//2$, plus spear bonus). A raider wounded there turns and flees empty-handed; one dropped to 0 HP is cut down. Only against an undefended camp does the raider loot --- \texttt{\_raid\_steal} takes 5 \texttt{food\_stock} (falling back to pawn rucksacks) --- after which it flees. Wounding a raider at any range sends it running, so the \texttt{Attack} action's \texttt{target} literal extends to \texttt{raider\_ids} too, and the \texttt{!raiders} god command lists active raiders and their stolen spoils.

\subsection{Discord ``God'' Interface}
Beyond the one-way webhook embeds, a two-way Discord bot (prefix \texttt{!}) exposes a god layer that manipulates the world directly (Table~\ref{tab:god}). Commands that mutate \texttt{world\_state} are serialised against the tick loop via an \texttt{asyncio.Lock}, and a \texttt{!pause}/\texttt{!resume} pair gates the scheduler. Enforced orders (\texttt{!order}) are injected into the prompt and overridden in code; whispers (\texttt{!say}) are injected into the prompt \emph{and} grant a deterministic $+15$ morale, letting the Creator pull an agent out of a mental break. \texttt{!graveyard} reads the permanent record of the fallen, \texttt{!tree} renders kinship and rivalries, \texttt{!wildlife} lists the fauna, \texttt{!visitors} lists the wandering travelers, \texttt{!raiders} lists the hostile scavengers, \texttt{!heirlooms} lists the relics of the fallen, \texttt{!monument} inspects the Ancestral Monolith, \texttt{!tradition} shows the colony's emergent Tradition Tag and its counters, \texttt{!chronicle} reads the seasonal lore, \texttt{!recipes} lists every known blueprint (base and Architect-synthesized), \texttt{!quests}/\texttt{!prophecies} show the active world objectives with progress, and \texttt{!patchnotes} reads the latest autonomous balance notes. Human watchers (not just the god) can also adopt a pawn from any channel via \texttt{!adopt} --- the mapping persists, \texttt{!my} shows the adopted ward, \texttt{!unadopt} releases it, and milestone events (birth, fulfilled goal, mental break, death) are delivered to the adopter as a direct message over the gateway, routed through a notifier hook (\texttt{core.notifier}) that \texttt{bot.on\_ready} registers, so \texttt{engine.py} stays free of Discord logic.
...
\texttt{!graveyard} & List the fallen with epithets and epitaphs (💖 marks the beloved) \\
\texttt{!tree} / \texttt{!wildlife} / \texttt{!visitors} / \texttt{!raiders} & Family tree / roaming fauna / travelers / hostile scavengers \\
\texttt{!heirlooms} / \texttt{!monument} / \texttt{!tradition} / \texttt{!chronicle} & Relics of the fallen / the monolith / emergent tradition / seasonal lore \\
...
def build_models():
    ...
    target_ids = (pawn_ids
                  ...
                  + [r["id"] for r in world_state.get("raiders", [])])
    AgentDecision = create_model(
        ...
        target=(Optional[Literal[tuple(target_ids)]], Field(
            default=None, description="Target pawn id (Attack/Share/Mate), wildlife id (Attack, taming), visitor id (Share, Mate/recruit, Attack), or raider id (Attack)"))),
...
\begin{lstlisting}[style=python, caption={Deterministic Resolution (engine.py, condensed). Applies action $\to$ consequences: energy costs, combat, skill XP, relationships, incapacitation, crafting, mental breaks, environmental death, Mate, free-form Interact, personal goals, traits, moodlets, fauna (hunt/tame, granary, palisade), visitors (barter, plunder, recruit), scavenger raids (steal, defend, flee), megaprojects (monument), farm plots (till/grow/harvest), emergent traditions (counters $\to$ Tradition Tag), solstice feasts &amp; funerary rites (Grief halving), wildfires, seasonal disasters, and Stage 9 synthesis (custom recipes, quests, clamped balance modifiers)}, label={lst:engine}]
    ...
    for pawn_id, intent in intents.items(): # (action, target, flavor, new_goal)
        ...
        if action == "Rest":
            ...
            if target in VISITOR_IDS:      # rob a traveler; gentle pawns feel guilt
                ...
                continue
            if target in RAIDER_IDS:       # fight off a scavenger
                dmg = max(1, 5 + pawn["skills"]["combat"] // 2)
                raider = raiders[target]; raider["hp"] -= dmg
                if raider["hp"] <= 0:
                    del raiders[target]; add_event("raid", action="slain")
                else:
                    raider["state"] = "fleeing"       # wounded raiders flee
                continue
...
def tick_environment():             # the world ages after pawns act
    ...
    step_visitors()
    # scavenger raids (Stage 8): a wealthy colony (>=30 food+wood) draws
    # 1-2 hostile raiders on an Autumn's first tick. They march to the Camp,
    # slowed by the palisade; tamed predators and high-combat pawns defend
    # the stores, else the raider steals 5 food and flees. Wounded raiders flee
    if season == "Autumn" and tick % 100 == 0 and not raiders \
            and colony_wealth() >= 30:
        spawn_raid(1 + randrange(2))
    step_raiders()
    ...
    # emergent traditions (Stage 7): colony-wide counters survive deaths;
    # on a season change the first threshold crossed (10 predators slain,
    # 100 trees felled, 20 rations shared -- priority order) stamps a sticky
    # Tradition Tag that tunes the engine until the end of the run
    traditions_inc("predators_slain") on predator kills
    traditions_inc("trees_felled") on every Chop
    traditions_inc("rations_shared") on Share (incl. visitor barter)
    if season changed and not traditions["tag"]:
        if predators_slain > 10:   tag = "Hunters of the North"
        elif trees_felled > 100:   tag = "Children of the Forest"
        elif rations_shared > 20:  tag = "Kindred of the Hearth"
        # Hunters: 2x hunting combat XP, cold penalties -1
        # Foresters: +1 wood/Chop, shelter damage halved
        # Kindred: social Interact +8 morale instead of +5
    # solstice feast (Stage 7): on the first tick of Winter/Summer, a full
    # larder (>15 food_stock) buys a colony-wide feast: -5 stock, every active
    # pawn +15 morale and a Festive moodlet (+5 for 15 ticks)
    if new_season in (Winter, Summer) and season just changed:
        if biome["food_stock"] > 15:
            biome["food_stock"] -= 5
            for p in active_pawns:
                p["vitals"]["morale"] += 15; moodlet(p, "Festive", +5, 15)
    # funerary rites (Stage 7): _kill marks the graveyard entry "beloved" when
    # survivors' average relationship >= 25; Interact "bury"/"mourn"/"eulogize"
    # at Camp or Ruins halves every Grief moodlet for same-tile active pawns
    if flavor matches rite words and tile in (Camp, Ruins):
        if any beloved in graveyard:
            for p in same_tile active pawns:
                for m in p["moodlets"] where m["name"] == "Grief":
                    m["ticks_left"] //= 2
...
\begin{lstlisting}[style=python, caption={Tick Orchestration (core.py, condensed). lock $\to$ schema $\to$ LLM $\to$ override $\to$ resolve $\to$ environment $\to$ tick++ $\to$ save; season-change detection, eulogy, adoption DMs, the Architect review, and embed all run outside the lock}, label={lst:tick}]
async def run_tick():
...
\begin{enumerate}
\item \textbf{Stable Long-Run Operation}: no OOM faults or state corruption occurred across extended runs; the 321-test offline suite (\texttt{tests/}) validates deterministic resolution, and state survived restarts via \texttt{terrarium\_state.json} while the structured event log (\texttt{terrarium\_log.jsonl}) grew append-only as the paper dataset.
...
\section{Conclusion}
The proposed architecture confirms that hosted open-weight models such as Gemma 4 can act as autonomous decision engines for background game loops while keeping the host footprint minimal, a 2 GB container with no model resident in RAM. Decoupling \emph{intent} (LLM) from \emph{consequence} (deterministic engine) prevents state corruption, while per-tick schema rebuilds (validated by the 370-test offline suite) passed as structured-output schemas and a ring buffer of structured events keep long-running behaviour coherent. A two-way Discord interface makes the system observable and steerable by a human ``god'' without breaking the simulation's determinism, and the pure-Python PNG map renderer (\texttt{map\_renderer.py}, built on \texttt{struct} and \texttt{zlib}) keeps the zero-framework ethos. Future work may explore self-hosting Gemma 4 locally via Ollama for fully-offline runs, dynamic biome adaptation, and multi-modal feedback via image generation.

\subsection{Future Work: Roadmap}
The design remains a staged roadmap beyond the community/lore layer of Stage 3, the hazard/visitor layers of Stages 4--5, the megaproject/farming layer of Stage 6, the culture/ritual layer of Stage 7, the raid/defense layer of Stage 8, and the autonomous-world-engine layer of Stage 9. Each stage keeps the AI Director pattern --- the LLM proposes intent only, while pure-Python deterministic rules own all numbers, state stays flat, and every mechanic remains offline-testable:

\begin{enumerate}
\item \textbf{Stage 9 --- The Autonomous World Engine (Dynamic Synthesis \& Balance Patches)}: implemented. Inspired by VRMMO autonomous game masters, the engine gained dynamic recipe discovery (\texttt{custom\_recipes} checked alongside static \texttt{RECIPES}; synthesized blueprints craftable via \texttt{Build} at tiers 4--10 with per-resource costs and combat/woodcutting/scouting/fiber bonuses), procedural world prophecies with multi-tick quest tracking (\texttt{active\_quests}: hunt/chop/stockpile/survive kinds paying colony-wide morale and optional titles), and bounded tuning multipliers (\texttt{biome.modifiers} for regrowth/cold/spawn rates). Every \texttt{PATCH\_INTERVAL} (400 ticks, one year) an Architect LLM reviews the world outside the tick lock and emits a \texttt{PatchUpdate} (\texttt{patch\_title}, \texttt{balance\_changes}, bounded numeric deltas, optional \texttt{new\_recipe}/\texttt{new\_quest}); Python validates and strictly clamps every value to $[0.7, 1.3]$ regardless of LLM output, increments \texttt{patch\_version}, appends to a capped \texttt{patches} ledger, logs a \texttt{patch} event, and posts a formatted ``Terrarium Patch Notes vX.Y'' embed --- all without rewriting raw \texttt{.py} code, keeping the deterministic engine as the sole source of truth.
\item \textbf{Stage 10 --- Persistent Narrative Memory (future)}: colony-authored artifacts (murals, journals, spoken legends) persist as physical world objects that seed future prompts, and an LLM-arbitrated ``World Soul'' council votes on each annual patch's direction, deepening autonomy while keeping numeric consequences inside the deterministic engine.
\end{enumerate}
