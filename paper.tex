...
\begin{enumerate}
\item \textbf{Stable Long-Run Operation}: no OOM faults or state corruption occurred across extended runs; the 263-test offline suite (\texttt{tests/}) validates deterministic resolution, and state survived restarts via \texttt{terrarium\_state.json} while the structured event log (\texttt{terrarium\_log.jsonl}) grew append-only as the paper dataset.
...
\begin{enumerate}
\item \textbf{Stage 6 --- Agriculture / Garden Plots}: Meadow tiles can be tilled into \texttt{Farm Plot}s that grow over 20 ticks (Spring/Summer only) and yield a guaranteed seasonal harvest without depleting natural stocks. (The megaproject half of Stage 6 --- the Ancestral Monolith --- is already implemented; see \S Implementation.)
