...
\begin{lstlisting}[style=python, caption={Deterministic Resolution (engine.py, condensed). Applies action $\to$ consequences: energy costs, combat, skill XP, relationships, incapacitation, crafting, mental breaks, environmental death, Mate, free-form Interact, personal goals, traits, moodlets, fauna (hunt/tame, granary, palisade), visitors (barter, plunder, recruit), megaprojects (monument), farm plots (till/grow/harvest), emergent traditions (predators slain / trees felled / rations shared $\to$ Tradition Tag), wildfires, and seasonal disasters}, label={lst:engine}]
...
\section{Conclusion}
The proposed architecture confirms that hosted open-weight models such as Gemma 4 can act as autonomous decision engines for background game loops while keeping the host footprint minimal, a 2 GB container with no model resident in RAM. Decoupling \emph{intent} (LLM) from \emph{consequence} (deterministic engine) prevents state corruption, while per-tick schema rebuilds (validated by the 289-test offline suite) passed as structured-output schemas and a ring buffer of structured events keep long-running behaviour coherent. A two-way Discord interface makes the system observable and steerable by a human ``god'' without breaking the simulation's determinism, and the pure-Python PNG map renderer (\texttt{map\_renderer.py}, built on \texttt{struct} and \texttt{zlib}) keeps the zero-framework ethos. Future work may explore self-hosting Gemma 4 locally via Ollama for fully-offline runs, dynamic biome adaptation, and multi-modal feedback via image generation.

\subsection{Future Work: Roadmap}
The design remains a staged roadmap beyond the community/lore layer of Stage 3, the hazard/visitor layers of Stages 4--5, and the megaproject/farming layer of Stage 6. Each stage keeps the AI Director pattern --- the LLM proposes intent only, while pure-Python deterministic rules own all numbers, state stays flat, and every mechanic remains offline-testable:

\begin{enumerate}
\item \textbf{Stage 7 --- Colony Ideology, Traditions \& Rituals}: emergent Tradition Tags (Hunters of the North, Children of the Forest, Kindred of the Hearth) already tune hunting, woodcutting, and social morale from colony-wide historical counters; Solstice Feasts and funerary rites (which halve \texttt{Grief} duration) remain to add communal texture.
