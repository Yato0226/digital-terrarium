...
\texttt{!heirlooms} / \texttt{!chronicle} & Relics of the fallen / seasonal lore \\
...
\begin{enumerate}
\item \textbf{Stable Long-Run Operation}: no OOM faults or state corruption occurred across extended runs; the 201-test offline suite (\texttt{tests/}) validates deterministic resolution, and state survived restarts via \texttt{terrarium\_state.json} while the structured event log (\texttt{terrarium\_log.jsonl}) grew append-only as the paper dataset.
...
\begin{enumerate}
\item \textbf{Stage 6 --- Base Megaprojects \& Cultivation}: once the camp is fully fortified, \texttt{Build} unlocks the Ancestral Monolith (20 wood + 15 stone, built progressively; completes with a morale floor and a \texttt{!monument} inscription written by an outside-lock LLM call), while Meadow tiles can be tilled into \texttt{Farm Plot}s that yield a guaranteed seasonal harvest without depleting natural stocks.
