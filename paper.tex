...
\texttt{!tree} / \texttt{!wildlife} & Family tree / roaming fauna \\
...
Listing~\ref{lst:schema} shows the per-tick dynamic schema; Listing~\ref{lst:engine} the deterministic resolution core; Listing~\ref{lst:tick} the tick orchestration. Together they implement the AI Director pattern. Note that \texttt{Literal[tuple(pawn\_ids + wildlife\_ids)]} is used deliberately: on Python 3.10 the starred \texttt{Literal[*ids]} unpacking form is unavailable, and the tuple form is equivalent per PEP~586.

\begin{lstlisting}[style=python, caption={Dynamic LLM Output Schema (schema.py, condensed). Uses \texttt{Literal[tuple(pawn\_ids + wildlife\_ids)]} for Python 3.10 compatibility (PEP 586)}, label={lst:schema}]
def build_models():
    pawn_ids = [pid for pid, p in world_state["pawns"].items()
                if p["status"] == "active"]            # roster-aware
    target_ids = pawn_ids + [w["id"] for w in world_state["wildlife"]]
    AgentDecision = create_model(
        ...
        target=(Optional[Literal[tuple(target_ids)]], Field(
            default=None, description="Target pawn id (Attack/Share/Mate) or wildlife id (Attack, taming)")),
...
The design remains a staged roadmap beyond the community/lore layer of Stage 3. Each stage keeps the AI Director pattern --- the LLM proposes intent only, while pure-Python deterministic rules own all numbers, state stays flat, and every mechanic remains offline-testable:

\begin{enumerate}
\item \textbf{Stage 4 --- Dynamic World Hazards \& Dynamic Tiles}: wildfires ignite from storms, Summer heatwaves, or Pyromaniac breaks and spread through a Forest $\to$ Burning $\to$ Ash $\to$ regrowth lifecycle (douseable via \texttt{Interact}, firebreaks via \texttt{Chop}); seasonal anomalies add flash floods (River expansion, then a silt food bonus), Winter aurorae (+10 colony morale with an atmospheric prompt line), and Autumn spore blooms in the Ruins (HP loss unless a \texttt{Warm Coat} is worn). \texttt{map\_renderer.py} gains fire and ash colours.
\item \textbf{Stage 5 --- Visitors, Caravans \& Wandering Nomads}: transient visitor entities (\texttt{world\_state["visitors"]}) spawn at the grid edge roughly every 150 ticks, pathfind to Camp, stay 3--5 ticks, then depart --- the Merchant (rare stocks), the Lost Wanderer, and the Wandering Bard (+5 morale at the campfire). \texttt{Share} barters resources, a high-sociability \texttt{Interact} can recruit a wanderer as a colonist under \texttt{MAX\_PAWNS}, and attacking a visitor plunders their inventory but grants a \texttt{Guilt} moodlet.
