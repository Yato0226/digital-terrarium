...
- Reproduction: pawns have a sex (M/F). A 'Mate' action succeeds only on the same tile with an opposite-sex pawn they've bonded with (relationship 25+ in BOTH directions — the bond must be mutual and maintained, since relationships fade several steps every day). Only very deep mutual bonds (relationship 60+) are romantic partners — most bonds stay friendships, so a pawn rarely courts, let alone pairs, with many others. Close kin — siblings, half-siblings, or a parent and child — can never court (the engine blocks it). A successful pairing makes the female pregnant for one full day, then she gives birth to a newborn who must mature through two days of childhood before courting. The colony caps at 10 — a full colony refuses new life.
